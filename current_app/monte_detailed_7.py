...
Incorporates improvements for:
  ...
  - Allowing a "random" option for:
       * Initial Betting Sequence (e.g. SSSRRR)
       * Tie Favored Pick (S or R)
       * Advantage Favored Pick (A=advantage, S=server, R=receiver)
"""
...
# Default settings for the betting sequence and picks
...
class ReportingManager:
    ...
    def compute_additional_analytics(self):
        ...
        # Corrected maximum drawdown %: (max drawdown / peak bankroll) * 100
...
def generate_detailed_analytics_report(reporting_manager, output_file):
    ...
    # Subplot 5: Executive Dashboard with Sparklines
...
def generate_combined_comparison_report(baseline_reporter, comp_reporter, output_file="monte_carlo_results_combined.pdf"):
    """
    Generate a combined PDF report that superimposes key charts from both baseline and comparison simulations.
    """
    # We'll produce two superimposed plots: Bankroll Evolution and ROI Distribution.
    
    # Combined Bankroll Evolution
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig("combined_bankroll.pdf", format='pdf', dpi=300, bbox_inches='tight')
    plt.close()
    
    # Combined ROI Distribution
    fig, ax = plt.subplots(figsize=(10,6))
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig("combined_roi.pdf", format='pdf', dpi=300, bbox_inches='tight')
    plt.close()
    
    # Now combine the above two PDF pages into one using matplotlib's PdfPages
    from matplotlib.backends.backend_pdf import PdfPages
    with PdfPages(output_file) as pdf:
        for fname in ["combined_bankroll.pdf", "combined_roi.pdf"]:
...
def main():
    ...
    # For these string parameters, allow "random" option.
    seq_input = prompt_for_string("Enter Initial Betting Sequence (e.g. SSSRRR)", "SSSRRR")
    if seq_input.lower() == "random":
        INITIAL_SEQUENCE = "".join(random.choice(["S", "R"]) for _ in range(6))
        print(f"Randomly generated Initial Betting Sequence: {INITIAL_SEQUENCE}")
    else:
        INITIAL_SEQUENCE = seq_input
    
    tie_input = prompt_for_string("Enter Tie favored pick (S or R)", "S")
    if tie_input.lower() == "random":
        TIE_FAVORED = random.choice(["S", "R"])
        print(f"Randomly chosen Tie favored pick: {TIE_FAVORED}")
    else:
    ...
    if adv_input.lower() == "random":
        ADVANTAGE_FAVORED = random.choice(["A", "S", "R"])
        print(f"Randomly chosen Advantage favored pick: {ADVANTAGE_FAVORED}")
    else:
    ...
    baseline_max_drawdowns = baseline_results["max_drawdowns"]
    ...
    print(f"Total Profit: Mean: {baseline_total_profits.mean():.2f} | Median: {np.median(baseline_total_profits):.2f} | Std: {baseline_total_profits.std():.2f} | Min: {baseline_total_profits.min():.2f} | Max: {baseline_total_profits.max():.2f}")
    print(f"ROI (%): Mean: {baseline_rois.mean():.2f}% | Median: {np.median(baseline_rois):.2f}% | Std: {baseline_rois.std():.2f}% | Min: {baseline_rois.min():.2f}% | Max: {baseline_rois.max():.2f}%")
    ...
    if compare_choice.startswith('y'):
        ...
        if comp_seq_input.lower() == "random":
            comp_INITIAL_SEQUENCE = "".join(random.choice(["S", "R"]) for _ in range(6))
            print(f"Randomly generated Initial Betting Sequence: {comp_INITIAL_SEQUENCE}")
        else:
        ...
        if comp_tie_input.lower() == "random":
            comp_TIE_FAVORED = random.choice(["S", "R"])
            print(f"Randomly chosen Tie favored pick: {comp_TIE_FAVORED}")
        else:
        ...
        if comp_adv_input.lower() == "random":
            comp_ADVANTAGE_FAVORED = random.choice(["A", "S", "R"])
            print(f"Randomly chosen Advantage favored pick: {comp_ADVANTAGE_FAVORED}")
        else:
            comp_ADVANTAGE_FAVORED = comp_adv_input
        
        # Update global variables with comparison settings
        ...
        comp_max_drawdowns = comparison_results["max_drawdowns"]
        ...
        print(f"Total Profit: Mean: {comp_total_profits.mean():.2f} | Median: {np.median(comp_total_profits):.2f} | Std: {comp_total_profits.std():.2f} | Min: {comp_total_profits.min():.2f} | Max: {comp_total_profits.max():.2f}")
        print(f"ROI (%): Mean: {comp_rois.mean():.2f}% | Median: {np.median(comp_rois):.2f}% | Std: {comp_rois.std():.2f}% | Min: {comp_rois.min():.2f}% | Max: {comp_rois.max():.2f}%")
        ...
        # Generate a combined comparison report with superimposed graphs.
