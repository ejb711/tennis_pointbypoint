...
Incorporates user feedback for:
  ...
  - Enhancing data visualization (improved binning, KDE overlays, error bars)
  - Improving report structure with composite metrics, scenario analysis, 
    and statistical significance testing
  - Adding betting pattern analysis (win/loss streaks)
  - Including details of the bet with the maximum wager in the executive dashboard
"""
...
def format_currency(amount):
    ...
    ANSI codes are removed from file outputs or CSV exports.
    """
    if amount > 0:
...
class ReportingManager:
    ...
    Handles the creation of a structured report:
      ...
      - Table of Contents
      - Detailed Analytics
      - Visual Enhancements (with an executive dashboard)
    ...
    def compute_additional_analytics(self):
        ...
        """
        # 1. Sharpe Ratio (simplistic approach)
        ...
        # 2. Max Drawdown Percentage (using total stakes as proxy)
    ...
    def compute_composite_metrics(self):
        ...
        Compute composite metrics:
          - Risk-adjusted return: overall ROI divided by the standard deviation of bet profits
          - Efficiency score: average profit per bet
          - Sustainability index: (final bankroll - initial bankroll) / max drawdown
        """
        returns = [bet.profit for bet in self.analytics.bets if bet.stake > 0]
    ...
    def compute_statistical_tests(self):
        """
        Perform a t-test comparing the profits of Progression vs. Score-Based strategies.
        """
        ...
        if len(progression_profits) > 1 and len(score_profits) > 1:
            stat, pvalue = ttest_ind(progression_profits, score_profits, equal_var=False)
    ...
    def create_executive_summary(self):
        ...
        summary = (
            ...
            "In brief, the system shows moderate performance. Further analysis (including scenario testing) is recommended to assess long-term viability.\n\n"
        )
        return summary
    ...
    def create_detailed_analytics_text(self):
        ...
        composite metrics, and interpretive commentary.
        """
        pick_text = ""
        ...
        details = (
            ...
            f" - Efficiency Score: {self.composite_metrics.get('Efficiency Score', 0):.2f}\n"
            f" - Sustainability Index: {self.composite_metrics.get('Sustainability Index', 0):.2f}\n\n"
            "The above metrics provide a multifaceted view of system performance.\n\n"
        )
        return details

    def create_practical_insights(self):
        """
        Summarizes practical insights and recommendations, including scenario analysis.
        """
        insights = (
            "PRACTICAL INSIGHTS & RECOMMENDATIONS\n"
            "=====================================\n\n"
            ...
            "- Scenario Analysis: Test under different market volatilities, altered odds structures, "
            "varied progression strategies, and alternative risk management approaches.\n\n"
        )
        return insights
...
def generate_detailed_analytics_report(reporting_manager, output_file="monte_carlo_results.png"):
    ...
    and Practical Insights. Also includes a final multi-plot figure (executive dashboard)
    with enhanced visualizations.
    """
    # Prepare structured text report
    text_report = []
    ...
    # Print final structured report to console
    print("\n" + "="*80)
    print(final_report_str)
    print("="*80 + "\n")
    
    # Define a cohesive, colorblind-friendly palette
    color_bankroll = "#0072B2"      # blue
    color_bet_size = "#009E73"      # green
    color_win_rate = "#0072B2"      # blue
    color_roi = "#D55E00"           # red
    color_bet_size_bins = "#56B4E9"  # light blue
    color_roi_hist = "#F0E442"      # yellow
    color_roi_mean = "#D55E00"      # red
    color_dashboard_text = "#000000" # black
    
    analytics = reporting_manager.analytics
    session_stats = reporting_manager.session_stats
    
    # Increase figure size to accommodate 7 subplots (executive dashboard)
    fig = plt.figure(figsize=(20, 35))

    # Subplot 1: Bankroll Evolution
    ax1 = plt.subplot(7, 1, 1)
    ax1.plot(analytics.bankroll_history, label='Bankroll', color=color_bankroll, linewidth=2)
    ...
    # Subplot 2: Bet Size Distribution (Enhanced Binning)
    ax2 = plt.subplot(7, 1, 2)
    if len(analytics.bet_sizes) > 0:
        bin_count = int(math.sqrt(len(analytics.bet_sizes)))
        ax2.hist(analytics.bet_sizes, bins=bin_count, color=color_bet_size, alpha=0.7)
        mean_bet = np.mean(analytics.bet_sizes)
        ax2.axvline(mean_bet, color=color_roi_mean, linestyle='--', label=f'Mean Bet: {mean_bet:.2f}')
    ...
    # Subplot 3: Strategy Performance with Error Bars
    ...
    for s in strategies:
        ...
        p_rate = (wins / total_bets * 100) if total_bets > 0 else 0
        win_rates.append(p_rate)
        # Binomial standard error for win rate
        err_wr = (100 * np.sqrt(p_rate/100*(1-p_rate/100)/total_bets)) if total_bets > 0 else 0
        ...
        # ROI standard error (using standard error of the mean)
        roi_list = [bet.roi for bet in analytics.bets if bet.strategy_type == s]
        err_roi = (np.std(roi_list) / np.sqrt(len(roi_list))) if roi_list and len(roi_list)>1 else 0
        roi_err.append(err_roi)
    
    ax3.bar(x - width/2, win_rates, width, yerr=win_rate_err, label='Win Rate %', color=color_win_rate, capsize=5)
    ax3.bar(x + width/2, roi_vals, width, yerr=roi_err, label='ROI %', color=color_roi, capsize=5)
    ax3.set_title('Strategy Performance Metrics (with Confidence Intervals)', fontsize=14, pad=20)
    ax3.set_xticks(x)
    ...
    if len(analytics.bet_sizes) > 0:
        ...
        # Use fixed 5 bins to ensure all bins are represented
        ...
        # Ensure bins 0-4 are shown even if some are missing
        groups = range(5)
        rates = [win_rates_by_group.get(i, 0) for i in groups]
        ax4.bar(groups, rates, color=color_bet_size_bins, alpha=0.7)
        ax4.set_xticks(list(groups))
    ...
    # Subplot 5: Executive Dashboard / Key Stats
    ax5 = plt.subplot(7, 1, 5)
    ax5.axis('off')
    max_bet = analytics.max_bet
    max_bet_text = "N/A"
    if max_bet:
        max_bet_text = f"Bet #{max_bet.bet_number} | Stake: ${max_bet.stake:.2f} | Outcome: {'WIN' if max_bet.won else 'LOSS'}"
    stats_text = (
        "EXECUTIVE DASHBOARD\n\n"
        ...
        f"Bet with Maximum Wager: {max_bet_text}\n\n"
        "Annotations on the charts above highlight key insights."
    )
    ax5.text(0.05, 0.5, stats_text, fontsize=12, color=color_dashboard_text, verticalalignment='center')

    # Subplot 6: ROI Distribution with KDE Overlay
    ax6 = plt.subplot(7, 1, 6)
    bet_rois = [bet.roi for bet in analytics.bets]
    if len(bet_rois) > 1:
        bin_count = int(math.sqrt(len(bet_rois)))
        counts, bins, patches = ax6.hist(bet_rois, bins=bin_count, color=color_roi_hist, alpha=0.7, density=True)
        # KDE overlay
        kde = gaussian_kde(bet_rois)
        x_vals = np.linspace(min(bet_rois), max(bet_rois), 200)
        ax6.plot(x_vals, kde(x_vals), color=color_roi_mean, linewidth=2, label='KDE')
        mean_roi = np.mean(bet_rois)
        ax6.axvline(mean_roi, color=color_roi_mean, linestyle='--', label=f"Mean ROI = {mean_roi:.2f}%")
    ax6.set_title('Distribution of Individual Bet ROI (with KDE Overlay)', fontsize=14, pad=20)
    ax6.set_xlabel('Bet ROI (%)')
    ax6.set_ylabel('Density')
    ax6.legend()
    ax6.grid(True, alpha=0.3)

    # Subplot 7: Betting Pattern Analysis (Win/Loss Streaks)
    ax7 = plt.subplot(7, 1, 7)
    win_streaks, loss_streaks = compute_streaks(analytics)
    # Prepare data for plotting streak frequencies
    win_streak_keys = sorted(win_streaks.keys())
    loss_streak_keys = sorted(loss_streaks.keys())
    win_vals = [win_streaks[k] for k in win_streak_keys]
    loss_vals = [loss_streaks[k] for k in loss_streak_keys]
    ax7.bar([k - 0.15 for k in win_streak_keys], win_vals, width=0.3, color="#009E73", label="Win Streaks")
    ax7.bar([k + 0.15 for k in loss_streak_keys], loss_vals, width=0.3, color="#D55E00", label="Loss Streaks")
    ax7.set_title('Betting Pattern Analysis: Win/Loss Streaks', fontsize=14, pad=20)
