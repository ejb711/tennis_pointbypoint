...
def compute_risk_profile(analytics, session_stats):
    ...
    # Kelly criterion (simplified): f* = p - (1-p)/b
    # p = overall win rate, b = net odds ratio (for server bets as example)
    ...
    # Dummy correlation: correlation between drawdown differences and subsequent profit changes
    drawdowns = np.array([session_stats['max_drawdown']])  # For illustration only
...
class ReportingManager:
    ...
    def __init__(self, analytics, session_stats):
        ...
        self.strategy_risk_adjusted = {}  # per strategy risk-adjusted returns
    ...
    def compute_additional_analytics(self):
        ...
        self.max_drawdown_pct = ((self.session_stats['total_stakes'] - self.session_stats['total_profit']) / total_stakes * 100) if total_stakes > 0 else 0.0
    ...
    def compute_composite_metrics(self):
        ...
        # Compute risk-adjusted return per strategy
    ...
    def create_executive_summary(self):
        ...
        if max_bet:
            max_bet_details = (f"Bet #{max_bet.bet_number} | Stake: ${max_bet.stake:.2f} | Outcome: {'WIN' if max_bet.won else 'LOSS'}\n"
                               f"Progression: {max_bet.pick} | Game Score: {max_bet.game_score}\n"
                               f"Game Sequence: {max_bet.game_sequence}")
        summary = (
            "EXECUTIVE SUMMARY\n"
            "==================\n\n"
            ...
            "Max Wager Details:\n"
            f"  {max_bet_details}\n\n"
            "In brief, the system shows moderate performance. Further scenario and risk profile analysis is recommended.\n\n"
        )
        return summary
...
def generate_detailed_analytics_report(reporting_manager, output_file="monte_carlo_results.png"):
    ...
    # Inline sparkline for Bankroll Trend
    ...
    # Inline sparkline for ROI Trend
    ...
    if len(bet_rois) > 1:
        ...
        # Annotate common spikes (e.g., -100% and +50%)
    ...
    # Cumulative probability line (illustrative)
    ...
    # Dummy data for sensitivity analysis: rows = SERVER_BET_MULTIPLIER values, columns = INITIAL_STAKE values
